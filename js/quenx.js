    ...
    if (wizard) {
        wizard.innerHTML = `
            <div class="error-message" style="text-align: center; padding: 40px; color: #ff6b6b;">
                <h3>Firmware Belum Tersedia | Terimakasih</h3>
                <p>Error: ${message}</p>
...
async function loadData() {
    try {
        ...
        try {
            ...
            throw new Error('Failed to parse JSON from decoded content');
        }
        
        if (!releases) {
            throw new Error('No data received after decoding');
        }
        ...
        if (!Array.isArray(releases)) {
            throw new Error('Decoded data is not an array format');
        }
